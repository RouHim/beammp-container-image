...
# Detect the latest PreRelease version
...
RUN export LATEST_VERSION=$(curl -s https://api.github.com/repos/BeamMP/BeamMP-Server/releases | grep -m 1 "tag_name" | cut -d '"' -f 4) && \
    export CURRENT_ARCH=$(uname -m | sed s/aarch64/arm64/g) && \
    export CURRENT_OS="ubuntu.24.04" && \
